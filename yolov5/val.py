...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        if opt.acc_threshold > 0.001:  # https://github.com/ultralytics/yolov5/issues/1466
            LOGGER.info(f'WARNING: confidence threshold {opt.acc_threshold} >> 0.001 will produce invalid mAP values.')
        run(**vars(opt))
        ...
        if opt.task == 'speed':  # speed benchmarks
            ...
            opt.acc_threshold, opt.iou_threshold, opt.save_json = 0.25, 0.45, False
